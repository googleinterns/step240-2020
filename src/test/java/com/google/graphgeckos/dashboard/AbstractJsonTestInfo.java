...
public abstract class AbstractJsonTestInfo {
  ...
  /**
   * The file names for one test differ only in prefixes. E.g input_real_json.txt and
   * output_real_json.txt.
   */
  ...
  /**
   * @param testName Common part of the input and output file names. E.g the test name "real_json"
   *     is expected to have input_real_json.txt file with input json and output_real_json.txt with
   *     expected output.
   */
  public AbstractJsonTestInfo(String testName) {
    ...
    try {
      String[] expected =
          readFile(getPath() + EXPECTED_PREFIX + testName + FILE_FORMAT).split("\n");
  ...
  /**
   * The expected output for each test is stored as a txt file. One line for each field. The order
   * of the values is described in inherited classes.
   *
